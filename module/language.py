...
translations = {
    "zhuanfa": ["转发", "Переслать", "Переслати"],
    ...
    "Forward video, use the method to directly enter /zhuanfa to view": [
        "转发视频，使用方法直接输入/zhuanfa查看",
        "Переслать видео, введите /zhuanfa для просмотра",
        "Переслати відео, введіть /zhuanfa для перегляду",
    ],
    "Listen forward, use the method to directly enter /jianting to view": [
        "监控转发，使用方法直接输入/jianting查看",
        "Слушать пересылку, введите /jianting для просмотра",
        "Слухати пересилання, введіть /jianting для перегляду",
    ],
