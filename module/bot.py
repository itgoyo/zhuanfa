...
class DownloadBot:
    ...
    async def start(
        ...
        commands = [
            ...
            types.BotCommand(
                "zhuanfa",
                _t("Forward video, use the method to directly enter /zhuanfa to view"),
            ),
            types.BotCommand(
                "jianting",
                _t(
                    "Listen forward, use the method to directly enter /jianting to view"
                ),
            ),
            types.BotCommand(
                "add_filter",
        ...
        self.bot.add_handler(
            MessageHandler(
                forward_messages,
                filters=pyrogram.filters.command(["zhuanfa"])
                & pyrogram.filters.user(self.allowed_user_ids),
            )
        )
        self.bot.add_handler(
        ...
        self.bot.add_handler(
            MessageHandler(
                set_jianting_msg,
                filters=pyrogram.filters.command(["jianting"])
                & pyrogram.filters.user(self.allowed_user_ids),
            )
        )
        self.bot.add_handler(
        ...
        self.client.add_handler(MessageHandler(jianting_msg))
...
async def send_help_str(client: pyrogram.Client, chat_id):
        ...
        [
            [
                InlineKeyboardButton(
                    "电报导航",
                    url="https://dianbaodaohang.com",
                ),
                InlineKeyboardButton(
                    "币圈导航", url="https://t.me/biquandaohang"
                ),
            ]
        ]
    )
    ...
    msg = (
        ...
        f"/zhuanfa - {_t('Forward messages')}\n"
        f"/jianting - {_t('Listen for forwarded messages')}\n"
...
async def forward_messages(client: pyrogram.Client, message: pyrogram.types.Message):
    ...
    async def report_error(client: pyrogram.Client, message: pyrogram.types.Message):
        ...
        await client.send_message(
            ...
            f"{_t('Please use')} "
            "/zhuanfa https://t.me/c/src_chat https://t.me/c/dst_chat "
            f"1 400 `[`{_t('Filter')}`]`\n",
        )
...
async def set_jianting_msg(
    ...
    if len(args) < 3:
        await client.send_message(
            message.from_user.id,
            f"{_t('Invalid command format')}. {_t('Please use')} /jianting "
    ...
    if node.chat_id in _bot.jianting_chat:
        _bot.remove_task_node(_bot.jianting_chat[node.chat_id].task_id)

    node.is_running = True
    _bot.jianting_chat[node.chat_id] = node


async def jianting_msg(client: pyrogram.Client, message: pyrogram.types.Message):
    """
    ...
    if message.chat and message.chat.id in _bot.jianting_chat:
        node = _bot.jianting_chat[message.chat.id]
    ...
    await client.send_message(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                ...
                [  # Second row
                    InlineKeyboardButton(
                        _t("Stop Listen Forward"), callback_data="stop_jianting"
                    )
                ],
            ]
        ),
